...
export default function App() {
  ...
  const addManualLocation = () => {
    setTableData(
      tableData.concat([[parseFloat(textLatitude), parseFloat(textLongitude)]])
    );
  };
  ...
  const sendPolygon = () => {
    ...
    polygon.OrganizationPolygonItem.CoordinatesFormated.push(
      polygon.OrganizationPolygonItem.CoordinatesFormated[0]
    );
    console.log(polygon);
    axios({
      method: "post",
      ...
      .then((response) =>
        Alert.alert(
          `${response.data.OrganizationPolygonItem.PolygonName} added`
        )
      )
    ...
    <View style={styles.container}>
      <View style={styles.topContainer}>
        <View style={styles.header}>
          <Image
            style={styles.pikud}
            source={require("./assets/pikud-logo.png")}
          ></Image>
          ...
          <Image
            style={styles.gasMask}
            source={require("./assets/gas-mask.png")}
          ></Image>
        </View>
        ...
        <View style={styles.body}>
          ...
          <Text style={styles.info}>
            במידה וזיהית זיהום בנקודת הציון הנוכחית, לחץ על כפתור שמור נקודה כדי
            להוסיפה לפוליגון.
          </Text>
          <TouchableOpacity
          ...
          <Text style={styles.subtitle}>
            נאספו {tableData.length} נקודות בפוליגון הנוכחי
          </Text>
          <Text
            style={styles.testButton}
            onPress={() => setManualVisible(true)}
          >
            הזן נקודת ציון ידנית
          </Text>
        </View>
      </View>
      ...
      <Dialog.Container visible={manualVisible}>
        ...
        <Dialog.Button
          ...
          disabled={!textLatitude || !textLongitude}
        />
      </Dialog.Container>
    </View>
  );
